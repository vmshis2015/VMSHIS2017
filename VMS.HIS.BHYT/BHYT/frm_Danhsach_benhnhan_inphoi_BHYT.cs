...
namespace VMS.HIS.BHYT
{
    public partial class FrmDanhsachBenhnhanInphoiBhyt : Form
    {
        ...
        KCB_BHYT _kcbBhyt = new KCB_BHYT();
            ...
            if (radChitiet.Checked)
            {
                try
                {
                    ...
                    foreach (GridEXRow row in grdList.GetCheckedRows())
                    {
                        ...
                        if (istao != "N")
                        {
                            ...
                            bool kt = _kcbBhyt.ProcessCreateXml(maluotKham, Utility.sDbnull(txtPath.Text.Trim()), congvan);
                            if (kt)
                            {
                                ...
                                i = i + 1;
                                LogText(string.Format("{0}. Xuất dữ liệu thành công bệnh nhân: {1}", maluotKham, DateTime.Now.ToString("HH:MM:ss")),
                                    Color.DarkBlue);
                                row.BeginEdit();
                                row.Cells["IsTao"].Value = "S";
                                row.EndEdit();
                            }
                            else
                            {
                                j = j + 1;
                                LogText(
                                    string.Format("{0}. Xuất dữ liệu không thành công bệnh nhân: {1} ", maluotKham, DateTime.Now.ToString("HH:MM:ss")),
                                    Color.Red);
                            }
                ...
                finally
                {
                    if (_xmlWriter917 != null)
                    {
                        ...
                        var file = new StreamWriter(Application.StartupPath + "\\PathXML.txt");
                        file.WriteLine(txtPath.Text.Trim());
                        file.Close();
                    }
                }
            }
            #region Ghi File tổng hợp
            else
            {
                Utility.ResetProgressBar(prgBar, grdList.GetCheckedRows().Count(), true);
                ...
                foreach (GridEXRow row in grdList.GetCheckedRows())
                {
                    ...
                    if (!string.IsNullOrEmpty(maphieudongttra))
                    {
                        ...
                        if (kt)
                        {
                            i = i + 1;
                            LogText(
                                string.Format("{0}. Insert thành công bệnh nhân vào bảng tạm: {1}",
                                    maluotKham, DateTime.Now.ToString("HH:MM:ss")), Color.DarkBlue);
                            new Update(KcbPhieuDct.Schema).Set(KcbPhieuDct.Columns.TrangthaiXml)
                                .EqualTo(1)
                        ...
                        else
                        {
                            j = j + 1;
                            LogText(
                                string.Format("{0}. Insert không thành công bệnh nhân vào bảng tạm: {1} ",
                                    maluotKham, DateTime.Now.ToString("HH:MM:ss")), Color.Red);
                        }
                    }
                    else
                    {
        ...
        {
            string duongdan = File.ReadAllText(Application.StartupPath + "\\PathXML.txt");
            if (!File.Exists(duongdan))
            {
                Process.Start("explorer.exe", duongdan);
            }
        }

        private void grdList_DoubleClick(object sender, EventArgs e)
        {
            string maLk = Utility.sDbnull(grdList.GetValue(KcbLuotkham.Columns.MaLuotkham));
            string isTao = Utility.sDbnull(grdList.GetValue("IsTao"));
            if (!string.IsNullOrEmpty(maLk) && isTao != "N")
            {
                frmViewDetail frm = new frmViewDetail(maLk);
                frm.ShowDialog();
            }
            else
            {
                Utility.ShowMsg("Bệnh nhân chưa có dữ liệu XML");
            }
        }
    }
}