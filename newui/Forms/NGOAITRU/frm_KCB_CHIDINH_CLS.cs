    ...
    public partial class frm_KCB_CHIDINH_CLS : Form
    {
        ...
        private void frm_KCB_CHIDINH_CLS_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                //                              DmucNhomcanlamsang.Columns.TenNhom
                //                          });
                string chuoi = " "+ DmucNhomcanlamsang.Columns.NguoiTao + " IN ('ADMIN', '"+globalVariables.UserName+"')";
                txtNhomDichvuCLS.Init(
                    globalVariables.gv_dtNhomDichVuCLS.Select(chuoi).CopyToDataTable(),
                    new List<string>()
                    {
                        DmucNhomcanlamsang.Columns.Id,
                        DmucNhomcanlamsang.Columns.MaNhom,
        ...
        private bool IsValidDataXoaCls()
        {
            ...
            foreach (GridEXRow gridExRow in grdAssignDetail.GetCheckedRows())
            {
                int assignDetailId =
                    Utility.Int32Dbnull(gridExRow.Cells[KcbChidinhclsChitiet.Columns.IdChitietchidinh].Value, -1);
                int i = m_dtChitietPhieuCLS.Select(KcbChidinhclsChitiet.Columns.IdChitietchidinh + " = " + assignDetailId +
                                          " And " + KcbChidinhclsChitiet.Columns.TrangThai + " >= 1").Count();
                //SqlQuery sqlQuery = new Select().From(KcbChidinhclsChitiet.Schema)
                //    .Where(KcbChidinhclsChitiet.Columns.IdChitietchidinh).IsEqualTo(AssignDetail_ID)
        ...
        private bool IsValidDataXoaCLS_Selected()
        {
            ...
            AssignDetail_ID =
                ...
                int k = m_dtChitietPhieuCLS.Select(KcbChidinhclsChitiet.Columns.IdChitietchidinh + " = " + AssignDetail_ID +
                                          " And " + KcbChidinhclsChitiet.Columns.TrangThai + " >= 1").Count();
                //SqlQuery sqlQuery = new Select().From(KcbChidinhclsChitiet.Schema)
                //    .Where(KcbChidinhclsChitiet.Columns.IdChitietchidinh).IsEqualTo(AssignDetail_ID)
                //    .And(KcbChidinhclsChitiet.Columns.TrangThai).IsGreaterThanOrEqualTo(1);
        ...
        string KiemtraCamchidinhchungphieu(int idDichvuchitiet,string tenChitiet)
        {
            ...
            DataRow[] arrDr = m_dtqheCamchidinhCLSChungphieu.Select(QheCamchidinhChungphieu.Columns.IdDichvu + "=" + idDichvuchitiet );
            DataRow[] arrDr1 = m_dtqheCamchidinhCLSChungphieu.Select(QheCamchidinhChungphieu.Columns.IdDichvuCamchidinhchung + "=" + idDichvuchitiet);
        ...
        private void grdAssignDetail_UpdatingCell(object sender, UpdatingCellEventArgs e)
        {
            try
            {
                if (e.Column.Key == KcbChidinhclsChitiet.Columns.SoLuong)
                {
                    ...
                    _row.Cells["TT_KHONG_PHUTHU"].Value = Utility.DecimaltoDbnull(_row.Cells[KcbChidinhclsChitiet.Columns.DonGia].Value, 0) * quanlitynew;
        ...
        private void cmdInPhieuCLS_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    KcbInphieu.InTachToanBoPhieuCls((int)objLuotkham.IdBenhnhan, objLuotkham.MaLuotkham, v_AssignId,
        ...
        private void InitData()
        {
            try
            {
                ...
                m_dtqheCamchidinhCLSChungphieu = globalVariables.gv_dtDmucQheCamCLSChungPhieu;
