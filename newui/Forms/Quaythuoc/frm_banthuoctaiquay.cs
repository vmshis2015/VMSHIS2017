...
namespace  VNS.HIS.UI.THANHTOAN
{
    public partial class frm_banthuoctaiquay : Form
    {
                ...
                if (objBenhnhan != null)
                    {
                      ...
                      if( !isValidPres( Pres_ID)) return;
                        ...
                        if (objPrescription != null)
                        {
                            ...
                            if (!frm.m_blnCancel)
                            {
                                Utility.GotoNewRowJanus(grdList, KcbDanhsachBenhnhan.Columns.IdBenhnhan, Utility.sDbnull(frm.objBenhnhan.IdBenhnhan,-1));
                                getData();
                                cmdThanhToan.Focus();
                                ModifyPresCommand();
                            }
        ...
        private void GetDataChiTiet()
        {
            try
            {
                long iddonthuoc = Utility.Int64Dbnull(grdList.GetValue("id_donthuoc"));
                m_dtChitietthanhtoan =
                    Quaythuoc.QuaythuocLaydonthuockhachhang(Utility.Int32Dbnull(iddonthuoc, -1));
        ...
        {
            long iddonthuoc = Utility.Int64Dbnull(grdChitiet.GetValue("Id_phieu"),-1);
            if (iddonthuoc > 0)
            {
                DataTable dtprint =null;
                    //SPs.QuaythuocLaythongtinIndonthuoc(iddonthuoc).GetDataSet().Tables[0];
                THU_VIEN_CHUNG.CreateXML(dtprint,"Quaythuoc_indonthuoc_A5.XML");
                Utility.UpdateLogotoDatatable(ref dtprint);
                string tieude = "",
                       reportname = "",
                       reportcode = "";
                reportcode = "Quaythuoc_indonthuoc_A5";
                ReportDocument crpt = Utility.GetReport("Quaythuoc_indonthuoc_A5", ref tieude,
                                                         ref reportname);
                if (crpt == null) return;
                var objForm = new frmPrintPreview("IN ĐƠN THUỐC", crpt, true, true);
                ...
                Utility.SetParameterValue(crpt, "CurrentDate",
                    string.Format(" {0} giờ {1} phút, Ngày {2} tháng {3} năm {4}", DateTime.Now.Hour, DateTime.Now.Minute,
                        dtPaymentDate.Value.Day, dtPaymentDate.Value.Month, dtPaymentDate.Value.Year));
                ...
                //In ngay
                if (chkPreviewHoadon.Checked)
                    objForm.addTrinhKy_OnFormLoad();
                if (Utility.isPrintPreview(PropertyLib._MayInProperties.TenMayInHoadon,
                                           PropertyLib._MayInProperties.PreviewInTomtatDieutriNgoaitru))
                {
                    objForm.SetDefaultPrinter(PropertyLib._MayInProperties.TenMayInHoadon, 0, 1);
                    objForm.ShowDialog();
                }
                else
                {
                    crpt.PrintOptions.PrinterName = PropertyLib._MayInProperties.TenMayInHoadon;
                    int soluongin = 1;
